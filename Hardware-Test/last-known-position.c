// This script saves the last known GPS position to an output file.

#include "esp_log.h"  // Add this at the top for ESP_LOGI()
#include <stdio.h>
#include <string.h>
...
// Optional: set NMEA sentence output type (for better fix)
    ...
    char last_fix[BUF_SIZE] = {0};  // Store most recent GPS fix string
    uint8_t data[BUF_SIZE];

    while (1) {
        // Send GPS status query
        uart_write_bytes(UART_PORT, "AT+CGNSINF\r\n", strlen("AT+CGNSINF\r\n"));
        uart_write_bytes(UART_PORT, "AT+CGNSSTAT?\r\n", strlen("AT+CGNSSTAT?\r\n"));
        ...
        if (len > 0) {
            ...
            if (strstr((char *)data, "+CGNSINF: 1,1,")) {
                strncpy(last_fix, (char *)data, sizeof(last_fix) - 1);
                last_fix[sizeof(last_fix) - 1] = '\0';  // Ensure null-termination
                ESP_LOGI("GPS", "Fix Acquired: %s", last_fix);
            } else {
                printf("Received (%d bytes): %s\n", len, (char *)data);
            }
        }
    
        vTaskDelay(pdMS_TO_TICKS(3000));  // Delay every loop, no matter what
    }
